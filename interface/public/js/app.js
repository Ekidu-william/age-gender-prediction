/**
 * app.js — Main application logic (index.html only)
 * ===================================================
 * Theme + settings + hamburger are handled by theme.js (loaded before this).
 * This file handles:
 *  - Tab switching (Upload / Camera)
 *  - File upload + drag-and-drop
 *  - Camera open/capture/close
 *  - Face detection via FaceDetect (face-detect.js)
 *  - Sending cropped 200x200 face to /predict
 *  - Displaying results
 */
    ...
    faceDetectLoop = setInterval(async () => {
        try {
            ...
            if (faceFound !== facePresent) {
                ...
                liveHint.textContent = faceFound ? '✓ Face detected — tap capture' : 'Center your face · Keep still';
...
btnAnalyze.addEventListener('click', async () => {
    if (!croppedBlob) { showToast('No face detected yet.', 'error'); return; }
    ...
    try {
        ...
        const msg = (err.message.includes('fetch') || err.message.includes('ECONNREFUSED'))
...
function showResults(data) {
    resThumb.getContext('2d').drawImage(cropCanvas, 0, 0, 80, 80);

    resAge.textContent = data.age;
    resGender.textContent = data.gender;
    resGenderSub.textContent = `${data.gender_confidence}% confidence`;

    const femalePct = data.gender === 'Female' ? data.gender_confidence : 100 - data.gender_confidence;
